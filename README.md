...
Агент превращает исторические данные в гипотезы, проверяет их шумными пилотами
и формирует портфель до 10 кампаний с учётом бюджета, контактов, стоимости
каналов и неопределённости эффекта.

## Документация

- [`PROJECT_GUIDE.md`](PROJECT_GUIDE.md) — полный разбор задачи, алгоритма,
  архитектуры, запуска, демонстрации и масштабирования.
- [`TESTING.md`](TESTING.md) — команды проверки и интерпретация результатов.
- [`PROGRESS.md`](PROGRESS.md) — журнал реализованных этапов и baseline.
...
```powershell
.\.venv\Scripts\python.exe -m unittest tests.test_priors
```
